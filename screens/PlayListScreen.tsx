...
const proxima = async () => {
  try {
    ...
    if (modoAleatorio) {
      if (lista.length === 1) {
        // Só uma música na lista, não toca nada
        setTocando(false);
        return;
      }
      ...
      if (currentTrackIndex + 1 >= lista.length) {
        // Última música da lista, para a reprodução
        setTocando(false);
        return;
      }
  ...
  return (
    ...
    >
      ...
      <View style={styles.buttons}>
        ...
        >
          <Feather name="shuffle" size={25} color={modoAleatorio ? "#FFD700" : "#000000ff"} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.buttonNext} onPress={anterior}>
          <Feather name="skip-back" size={25} color="#000000ff" />
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          {tocando ? (
            <View style={styles.view}>
              <Feather name="pause" size={25} color={"#00000096"} />
            </View>
          ) : (
            <View style={styles.view}>
              <Feather name="play" size={25} color={"#000000ff"} />
            </View>
          )}
        </TouchableOpacity>

        <TouchableOpacity style={styles.buttonNext} onPress={proxima}>
          <Feather name="skip-forward" size={25} color="#000000ff" />
        </TouchableOpacity>
        <TouchableOpacity
